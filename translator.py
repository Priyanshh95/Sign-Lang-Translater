...
def main2(sentence):
   ...
   if '{' and '}' in sentence:
       ...
       for i in L1:
           if '{' and '}' in i:
               find_anonymous(i)
       return ""  # If no output sentence, return an empty string or relevant value
   else:
       output_sentence = svo_to_sov(sentence)
   return output_sentence  # Only return the ISL sentence, not multiple values
# Path to the video folder (set as a global variable)
VIDEO_FOLDER = "static/videos"
def main3(text):
    """
    Converts the input text into ISL and matches it to existing video filenames.
    Returns a list of matched video filenames.
    """
    matched_videos = []  # To store filenames of matched videos
    L2 = []  # ISL sentence list
    L = nltk.sent_tokenize(text)  # Tokenize input into sentences

    for sentence in L:
        isl_translation = main2(sentence)  # Get ISL translation for the sentence
        L2.append(isl_translation)
        
        # Check for video corresponding to the ISL translation
        isl_words = isl_translation.split()  # Split ISL sentence into words
        for word in isl_words:
            video_filename = f"{word}.mp4"  # Video file expected for this word
            video_path = os.path.join(VIDEO_FOLDER, video_filename)
            if os.path.exists(video_path):  # Check if video file exists
                matched_videos.append(video_filename)

    return {
        "isl_sentence": '\n'.join(L2),  # Full ISL translation
        "matched_videos": matched_videos  # List of video filenames
    }
# def main3(text):
#    L2 = []  # Only keep the ISL sentence list
#    L = nltk.sent_tokenize(text)
#    for i in L:
#        y = main2(i)  # Only extract ISL sentence
#        L2.append(y)
#    output_sentence = '\n'.join(L2)  # Combine ISL sentences
#    return output_sentence
